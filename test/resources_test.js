...
contract('Game Store Resources', async (accounts)=>{
    let inventory;
    let DBT;
    let store;
    let game;
   ...
   it("Should buy each augment", async () => {
        ...
        for (let i = 0;i < parseInt((await inventory.getResourcesAmount()).toString());i++){
            await store.buyResources(i, 1, {from: user})
        }
        let resources = await inventory.getResources(user,{from: user})
        let balance = await DBT.balanceOf(store.address)
        assert.equal(balance, web3.utils.toWei("10") * parseInt((await inventory.getResourcesAmount())), "Balances equal")
        for (let i = 0;i < parseInt((await inventory.getResourcesAmount()).toString());i++){
            assert.equal(resources[i].toString(), 1, "Augment equal")
        }
    })
